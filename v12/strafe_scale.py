...
tms = gremlin.input_devices.JoystickDecorator( TM_STICK_NAME, gremlin.common.DeviceIdentifier(TM_STICK_HWID,TM_STICK_ID) , MODE_ALL )
...
def toggle_nsw(event, vjoy):
	...
	# gremlin.util.log("Toggle nose wheel steering: %s" % (event.is_pressed))
	toggle_nose_wheel = event.is_pressed
...
def xaxis(event, vjoy):
	...
	if toggle_nose_wheel:
		...
		vjoy[1].axis(1).value = curve(event.value)
		
# pitch axis disconnect of nose wheel steer
@tms.axis(2)
def xaxis(event, vjoy):
	global toggle_nose_wheel
	if not toggle_nose_wheel:
		vjoy[1].axis(2).value = curve(event.value)		
